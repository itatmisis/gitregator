...
public sealed class GithubAggregatorService : IGithubAggregatorService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var contibs = contributors.Select(x => x.Login);
        var users = new List<Collaborator>();
        var usersTasks = contibs.Select(con => client.User.Get(con)).ToList();
        var allRepositoryCommits = await client.Repository.Commit.GetAll(owner, name);
        var openRepositoryIssues = await client.Issue.GetAllForRepository(owner, name);
        var openRepositoryPullRequests = await client.PullRequest.GetAllForRepository(owner, name);
        var repositoryLanguages = await client.Repository.GetAllLanguages(owner, name);

        var usersGit = await Task.WhenAll(usersTasks);
        var tasksRepo = usersGit.Select(u => client.Repository.GetAllForUser(u.Login)).ToList();
        var repos = (await Task.WhenAll(tasksRepo)).SelectMany(x => x).ToList();
        ...
        users.AddRange(usersGit.Select(x =>
        {
            ...
            return new Collaborator()
            {
                ...
                RepositoryTotal = repos.Count(t => t.Owner.Login == x.Login),
                ActivityIndex = (decimal)allRepositoryCommits.Count(t => t?.Committer?.Login == x.Login) / 10
        ...
        return
            new GetRepositoryAggregationResponse
            {
                ...
                Collaborators = users,
                //Issuers = issuersDto,
                AllRepositoryCommits = allRepositoryCommits.Count,
                OpenIssues = openRepositoryIssues.Count,
                OpenPullRequests = openRepositoryPullRequests.Count,
    ...
    var tasks22Result = await Task.WhenAll(tasks22);*/
        ...
        worksheet.Cell(rowCount, 2).Value = allRepositoryCommits.Count;
        rowCount++;
        worksheet.Cell(rowCount, 1).Value = "Всего пулл реквестов:";
        worksheet.Cell(rowCount, 2).Value = openRepositoryPullRequests.Count;
        rowCount++;
        ...
        worksheet.Cell(rowCount, 7).Value = "Кол-во репозиториев";
        ...
        foreach (var user in users)
        {
            ...
            worksheet.Cell(rowCount, 7).Value = user.RepositoryTotal;
        ...
        CancellationToken cancellationToken)
    {
        ...
        var user = new ExtendedUser()
        {
            ...
            StackOverdlowReputationChange = stackExchangeUser?.ReputationChangeYear
        };
